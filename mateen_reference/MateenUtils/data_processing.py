...
def prepare_data(scenario="IDS2017"):
    if scenario == "IDS2017":
        ...
        train_path = "Datasets/Mirai/TrainData.csv"
        test_path = "Datasets/Mirai/TestData.csv"
        train = load_dataset(train_path)
        print(f'Train Loaded with {len(train)} Samples')
        test = load_dataset(test_path)
        print(f'Test Loaded with {len(test)} Samples')
    elif scenario in ["ARP_MitM", "OS_Scan", "SSDP_Flood", "Video_Injection", "Fuzzing", "Active_Wiretap", "SYN_DoS", "SSL_Renegotiation"]:
        train_path = f"Datasets/{scenario}/TrainData.csv"
        test_path = f"Datasets/{scenario}/TestData.csv"
        train = load_dataset(train_path)
        print(f'Train Loaded with {len(train)} Samples')
        test = load_dataset(test_path)
        print(f'Test Loaded with {len(test)} Samples')
