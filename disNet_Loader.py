...
class Task1_data_loader():
	...
	def __init__(self, poison_persent = 0, training_fraction = 0.6, placement_percent = 0, 
				shuffle = False, noise_from_class = 1, mask=[0,1,12,13,40], add_synthetic=True):   # 13 is timestamp
		...
		data_subfolder = 'odered_data'
		self.data_files = ["task_1/"+data_subfolder+"/no_interference.csv",  	# normal 
							"task_1/"+data_subfolder+"/nr_11dBm.csv", 			# nr_jamming
							"task_1/"+data_subfolder+"/lte_ul_11dBm.csv", 		# 11_dB_lte_ul_jamming
							"task_1/"+data_subfolder+"/lte_ul_5dBm.csv", 		# 5_dB_lte_ul_jamming 
							"task_1/"+data_subfolder+"/lte_ul_0dBm.csv" 		# 0_dB_lte_ul_jamming  
						,	"task_1/"+data_subfolder+"/lte_dl_11dBm.csv",		# 11_dB_lte_dl_jamming
							"task_1/"+data_subfolder+"/lte_dl_5dBm.csv",		# 5_dB_lte_dl_jamming
							"task_1/"+data_subfolder+"/lte_dl_0dBm.csv"			# 0_dB_lte_dl_jamming 
					]
...
class Task1():
	...
	def train_and_predict(self):
		...
		AnomDetector = NET(self.num_features,max_autoencoder_size,FM_grace_period,AD_grace_period,learning_rate,hidden_ratio, corruption_level)
	...
	def plot_results(self, threshold = None, fig = True): #threshold set by inspection
		...
		colors_set = ['green','red','blue','purple', 'yellow','cyan','brown','orange']
		...
		for i, file in enumerate(self.data_files):		
			plt.scatter([-1],[-1],s=3, marker='o', color=colors_set[i] , label=file[19:-4])
	...
	def calc_roc(self, ):
		...
		results = self.results_raw[self.training_size:]
...
def main():
	...
	if use_tex_backend:
		matplotlib.use('TkAgg')
	# plt.ion()
	# explore_data(poison_persent = 0, training_fraction = 0.66, placement_percent = .5, plot_jamming_classes = True, plot_stats= False, TSNE = False, use_gen = False)
